...
export async function get_question(game: number, question_id: number): Promise<Question | undefined> {
	return openDb().then((db) => db.get<Question>("SELECT id, lang, question FROM questions WHERE id = ?", question_id));
}
